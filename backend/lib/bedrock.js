...
export const TRUTH_SYSTEM_PROMPT = [
  ...
  "If a source clearly implies an obligation but its deadline or relation to an existing item is uncertain, return the obligation with currentDeadline null and a concise review object. For example, 'the assignment will be checked in OS class' is actionable; without 'next OS class', do not assert which class. Recommend a safe option and say what is uncertain. Never silently merge a merely similar title into a tracked task. When the obligation is tied to a class, lecture, or lab occurrence rather than a date ('sir will check it during OS', 'have it ready for class', 'evaluated in the next lecture'), keep currentDeadline null and set review.classReference with the subject named (or null) and explicit true only when the notice says which occurrence (for example 'next'). You never look up the timetable and never choose a date for a class. State the uncertainty briefly; never include your reasoning process.",
